...
class TestISAF12Builder(unittest.TestCase):
    ...
    def test_build_header_sets_file_version(self):
        ...
        self.assertTagText(header, "./FileDescription/FileVersion", "iSAF1.2")
    ...
    def test_build_header_sets_software_name(self):
        header = self.builder._build_header()
        self.assertTagText(header, "./FileDescription/SoftwareName", "Imone")

    def test_build_header_sets_software_version(self):
        header = self.builder._build_header()
        self.assertTagText(header, "./FileDescription/SoftwareVersion", "1.2")
    ...
    def test_builder_master_supplier_data_is_correct(self):
        master = self.builder._build_master_files()
        self.assertTagText(master, "./Suppliers/Supplier[1]/SupplierID", "1015")
        self.assertTagText(
            master,
        ...
        self.assertTagText(
            master,
            "./Suppliers/Supplier[1]/RegistrationNumber",
            "302222220",
        )
        self.assertTagText(master, "./Suppliers/Supplier[1]/Country", "LT")
        self.assertTagText(master, "./Suppliers/Supplier[1]/Name", 'UAB "Įmonė"')
    ...
    def test_builder_no_payments_SettlementsAndPayments_tag_not_added(self):
        ...
        self.assertEqual(len(source_docs.findall("./SettlementsAndPayments")), 0)

    def test_builder_source_docs_adds_two_purchase_invoices(self):
        source_docs = self.builder._build_source_documents()
        self.assertEqual(len(source_docs.findall("./PurchaseInvoices/Invoice")), 2)

    def test_builder_source_docs_adds_two_sales_invoices(self):
        source_docs = self.builder._build_source_documents()
        self.assertEqual(len(source_docs.findall("./SalesInvoices/Invoice")), 2)

    def test_builder_source_docs_adds_two_settlements_and_payments(self):
        source_docs = self.builder._build_source_documents()
        self.assertEqual(
            len(source_docs.findall("./SettlementsAndPayments/SettlementAndPayment")), 2
        )
    ...
    def test_dump_returns_correct_xml(self):
        ...
        self.assertEqual(len(isaf_xml.findall(xpath("./{%s}SourceDocuments"))), 1)
    ...
    def test_purchase_invoice_totals_amount_None_rendered_has_attr_xsi_nil(self):
