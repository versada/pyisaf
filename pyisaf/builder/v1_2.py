...
class ISAF1_2Builder(ISAFBuilder):
    ...
    def _build_header(self):
        ...
        SubElement(file_desc, "SoftwareCompanyName").text = fd["software_company_name"]
        SubElement(file_desc, "SoftwareName").text = fd["software_name"]
        SubElement(file_desc, "SoftwareVersion").text = fd["software_version"]
        SubElement(file_desc, "RegistrationNumber").text = int_or_empty(
            fd["registration_number"]
        )
        int_or_nil(SubElement(file_desc, "NumberOfParts"), fd.get("number_of_parts"))
        SubElement(file_desc, "PartNumber").text = fd["part_number"]

        period = SubElement(file_desc, "SelectionCriteria")
        SubElement(period, "SelectionStartDate").text = sc[
            "selection_start_date"
        ].strftime(DATE_FORMAT)
        SubElement(period, "SelectionEndDate").text = sc["selection_end_date"].strftime(
            DATE_FORMAT
        )
        return header
    ...
    def _build_settlement_and_payment(self, sp):
        ...
        SubElement(elem, "SettlementDate").text = sp["settlement_date"].strftime(
            DATE_FORMAT
        )
    ...
    def _build_payment(self, p):
        ...
        SubElement(elem, "vat_registration_number").text = p["vat_registration_number"]
    ...
    def _build_payment_invoice_reference(self, inv_ref):
        ...
        SubElement(elem, "InvoiceDate").text = inv_ref["invoice_date"].strftime(
            DATE_FORMAT
        )
    ...
    def _build_invoice_reference(self, ref):
        ...
        SubElement(elem, "ReferenceDate").text = ref["reference_date"].strftime(
            DATE_FORMAT
        )
        return elem
    ...
    def _build_sales_invoice_total(self, t):
        ...
        str_or_nil(SubElement(elem, "TaxPercentage"), t.get("tax_percentage"))
        str_or_nil(SubElement(elem, "Amount"), t.get("amount"))
        date_or_nil(SubElement(elem, "VATPointDate2"), t.get("vat_point_date2"))
        return elem
    ...
    def _build_purchase_invoice(self, inv):
        ...
        SubElement(elem, "InvoiceDate").text = inv["invoice_date"].strftime(DATE_FORMAT)
        ...
        registration_account_date_elem = SubElement(elem, "RegistrationAccountDate")
        date_or_nil(
    ...
    def _build_customer_supplier(self, cs, tag, id_tag, id_field):
        ...
        SubElement(elem, "RegistrationNumber").text = cs["registration_number"] or ""
